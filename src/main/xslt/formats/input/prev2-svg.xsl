...
<xsl:transform version="2.0"
  ...
  <xsl:template match="xr:bucket" mode="svg:sample">
    <xsl:variable name="bucket" select="tokenize(@xr:bbox, $xr:delimiter)" as="xs:string*"/>
    <xsl:variable name="bucketTop" select="xs:integer(subsequence($bucket, $xr:top, 1))" as="xs:integer"/>
    <xsl:variable name="bucketLeft" select="xs:integer(subsequence($bucket, $xr:left, 1))" as="xs:integer"/>
  ...
  <xsl:template match="xr:sample" mode="svg:sample">
    ...
    <xsl:variable name="sampledPolygons" as="element()*">
      <xsl:sequence select="for $mp in $bucket/svg:* return svg:sampledPolygons($mp, @x, @y)"/>
    </xsl:variable>
    
    <!-- <xsl:message>sampledPolygons = <xsl:value-of select="count($sampledPolygons)"/></xsl:message> -->
  ...
  <xsl:function name="svg:sampledPolygons" as="element()?">
    <xsl:param name="polygon" as="element()"/>
    <xsl:param name="sampleX" as="xs:double"/>
    <xsl:param name="sampleY" as="xs:double"/>
    <xsl:variable name="points" select="tokenize($polygon/@points, $svg:pointDelimiter)" as="xs:string*"/>
    <xsl:variable name="xCoords" select="for $p in $points return 
        number(subsequence(tokenize($p, $svg:coordDelimiter), $X, 1))" as="xs:double*"/>
    <xsl:variable name="yCoords" select="for $p in $points return 
        number(subsequence(tokenize($p, $svg:coordDelimiter), $Y, 1))" as="xs:double*"/>
